...
function setup() {
  ...
  //creating balloon     
  balloon = createSprite(width-70, 200, 20, 50);
...
function draw() {
  ...
  if (gameState === 'start') {
    background(bg2)
    textSize(45)
    text('Infinite balloon runner game', width/2-100, height - 500)
    text('Press enter to start', width/2 -100, height - 600)
    if (keyDown('enter')) {
      gameState = 'play'
    }
  }
...
function spawnlamp() {
  if (frameCount % 87 === 0) {
    lamp = createSprite(-5, height - 150, 300, 500);
    lamp.velocityX = 4
...
function spawncoins() {
  if (frameCount % 50 === 0) {
    coins = createSprite(-10, height - 200)
...
function spawnBuilding1() {
  if (frameCount % 87 === 0) {
    building1 = createSprite(-5, height - 150, 400, 600);
